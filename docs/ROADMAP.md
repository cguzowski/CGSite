...
| --- | --- | --- |
| F7 / foundation + Projects | Prioritize visible media | Fuji remains unchanged and starts without competition from offscreen photos or Swiper. About portrait loads near About; Projects initializes near its section and requests only viewed/adjacent cards. Responsive image candidates preserve full-resolution choices, use lossless compression where smaller, account for crop and display density, and retain original sources. No added poster transition. Slider controls, iOS fallback, reduced motion and native section links remain functional. Tests/build and generated-media validation pass; record measured bytes separately from unmeasured live timings. |
...
| F6 / foundation | Mobile section title navigation | At widths up to 48rem, About, Projects and Contact titles become content-sized, flat-oval sticky navigation bars only after reaching the viewport top within their sections; later headings remain regular centered titles until consumed by scrolling. The oval hugs the collapsed four-position/current-title pattern and expands to contain a cleanly spaced Home/About/Projects/Contact link row with the current section highlighted. Selection collapses after native navigation; Escape collapses with focus returned. Home and desktop keep their existing presentation. No horizontal overflow at 375px; keyboard controls work. |
...
| D1 / foundation | Automated static deployment | Pull requests and `master` pushes install locked dependencies, test, and build on GitHub-hosted runners. Successful `master` builds publish only the static browser artifact; a labeled Pi runner installs it as an atomic release, verifies local Nginx serving, retains rollback releases, and requires no inbound SSH access. Production deployment is environment-gated and never runs for pull requests. |
