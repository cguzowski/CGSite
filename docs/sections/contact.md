...
| Contact-only original SVGs | [contact-icons](../../angular/public/assets/icons/contact-icons) |
| Reused About SVGs (no Contact copies) | [profile-icons](../../angular/public/assets/icons/profile-icons) |
...
C1 presents six responsive icon-and-text connections with the icon above its paired copy: New York City map, direct email, LinkedIn, GitHub, downloadable resume and the retained published phone number. Map and profile links open in a new tab with opener isolation; email and phone use `mailto:` and `tel:`; the centered Back to top link returns to `#home`. Pin, mail and phone use original Contact SVGs. Resume, GitHub and LinkedIn reference the exact About SVGs under `profile-icons` rather than duplicating them.
