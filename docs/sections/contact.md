...
C1 presents six responsive icon-and-text connections with the icon above its paired copy, in this order: New York City map, direct email, the retained published phone number, GitHub, resume and LinkedIn. Map, resume and profile links open in a new tab; email and phone use `mailto:` and `tel:`; the centered arrow link returns to `#home`. Pin, mail and phone use original Contact SVGs. Resume, GitHub and LinkedIn reference the exact About SVGs under `profile-icons` rather than duplicating them.
